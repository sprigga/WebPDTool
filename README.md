...
|------|------|
| **版本** | v0.9.0 |
| **完成度** | ~98%（核心功能完成，測量服務完整，27 種儀器驅動實現，儀器管理 CRUD 完成，使用者管理完成，統計分析功能完成，Async SQLAlchemy 遷移完成，Modbus 整合完成，Modbus 自動觸發測試完成，TestResults 三合一視圖完成；SystemConfig 為預留 stub） |
| **最新更新** | 2026-03-26 — TestResults 三合一重構（合併 TestHistory + ReportAnalysis 為 3 個 Tab）、移除已廢棄路由 |
...
- ✅ **現代化前端** — Vue 3 Composition API + Element Plus UI，10 個視圖 + 4 個組件（9 個視圖功能完整，1 個 stub）
- ✅ **TestResults 三合一** — 查詢結果 / 歷史趨勢 / 報表分析整合為單一頁面的 3 個 Tab
...
| **字元集** | utf8mb4 |
| **資料表** | 10 個表（users、projects、stations、test_plans、test_sessions、test_results、configurations、sfc_logs、modbus_logs、instruments） |
| **ORM 模型** | 9 個（configurations / modbus_logs 為純 SQL，無對應 Model；modbus_config 有 Model 但透過 Alembic 管理） |
| **Alembic 遷移** | 7 個遷移版本 |
...
`runAllTest` 模式：失敗後繼續執行，收集所有錯誤，最後彙報。

### Modbus 自動觸發流程

```
選擇 station → TestMain.vue 自動連接 WS /api/modbus/ws/{station_id}
...
| 路由 | 視圖元件 | 功能 | 狀態 |
|------|---------|------|------|
| `/login` | Login.vue | 登入頁面（兩階段：帳密 → 專案站別選擇） | ✅ 完整 |
| `/main` | TestMain.vue | 測試執行主介面（含 SFC/runAllTest/OPjudge 設定，Modbus 快捷入口） | ✅ 完整 |
| `/test` | TestExecution.vue | 測試執行監控（進度/中止/重試） | ✅ 完整 |
| `/results` | TestResults.vue | **三合一視圖**：查詢結果 / 歷史趨勢 / 報表分析（ECharts 時間軸 + 統計圖表） | ✅ 完整 |
| `/testplan` | TestPlanManage.vue | 測試計劃管理（CSV 匯入/CRUD） | ✅ 完整 |
| `/projects` | ProjectManage.vue | 專案站別管理 | ✅ 完整 |
| `/users` | UserManage.vue | 使用者管理（Admin CRUD） | ✅ 完整 |
| `/instruments` | InstrumentManage.vue | 儀器設定管理（DB-backed CRUD） | ✅ 完整 |
| `/modbus-config` | ModbusConfig.vue | Modbus 配置管理 | ✅ 完整 |
| `/config` | SystemConfig.vue | 系統配置 | 🚧 Stub |

> **v0.9.0 變更**：原 `/history`（TestHistory.vue）和 `/analysis`（ReportAnalysis.vue）已合併至 `/results`（TestResults.vue）的 3 個 Tab 頁。路由和導航按鈕已移除。
...
| 站別管理 | `/api/stations` | 站別配置管理、testplan-names 查詢 |
| **儀器管理** | `/api/instruments` | **儀器 CRUD（DB-backed）** |
| 測試執行 | `/api/tests` | 會話控制、狀態查詢、儀器重置 |
| 測量執行 | `/api/measurements` | 測量調度、儀器控制、參數驗證 |
| **Modbus 管理** | `/api/modbus` | **配置 CRUD、狀態查詢、監聽器控制** |
| **Modbus WebSocket** | `/api/modbus/ws` | **即時事件推播（per-station）** |
...
│   │   │   ├── exceptions.py  # 自定義例外
│   │   │   ├── measurement_constants.py # 測量相關常數
...
├── frontend/                  # Vue 3 前端應用
│   ├── src/
│   │   ├── views/             # 頁面組件 (10 個視圖，9 個功能完整，1 個 stub)
│   │   │   ├── Login.vue
│   │   │   ├── TestMain.vue           # 測試執行主介面
│   │   │   ├── TestExecution.vue      # 測試執行監控
│   │   │   ├── TestResults.vue        # 三合一視圖（查詢結果 / 歷史趨勢 / 報表分析）
...
│   │   │   ├── testResults.js         # 測試結果查詢 API
│   │   │   ├── analysis.js            # 統計分析 API
│   │   │   ├── instruments.js         # 儀器 API
...
│   │   │   ├── users.js
│   │   │   └── instruments.js
│   │   ├── composables/       # Composition API 可組合函數 (3 個)
│   │   │   ├── useMeasurementParams.js
│   │   │   ├── useTestHistory.js
│   │   │   └── useTestTimeline.js
│   │   ├── router/            # Vue Router 配置 (10 條路由)
│   │   ├── App.vue            # 根組件
│   │   └── main.js            # 應用入口
│   └── package.json
├── database/                  # 資料庫設計
│   ├── schema.sql             # 資料庫 Schema（10 個表）
│   ├── seed_data.sql          # 初始資料
│   └── seed_instruments.sql   # 儀器設定初始資料
├── docs/                      # 文件（架構、功能、部署、API 分析）
├── screenshots/               # UI 截圖
...
# ⚠️ 本機開發必須使用 port 8765（Vite proxy target 為 localhost:8765）
...
| Backend (Docker) | 9100 | uvicorn FastAPI |
| Backend (local dev) | **8765** | Vite proxy target，本機開發必須用此 port |
...
| `GET` | `/api/stations/{station_id}/testplan-names` | 查詢測試計劃名稱列表 |
...
| `POST` | `/api/modbus/start/{station_id}` | 啟動 Modbus 監聽器 |
| `POST` | `/api/modbus/stop/{station_id}` | 停止 Modbus 監聽器 |
| `WS` | `/api/modbus/ws/{station_id}` | WebSocket 即時事件推播（含 stale socket guard） |
...
| 功能模組 | 完成度 | 說明 |
|---------|--------|------|
| **資料庫設計** | 100% | 9 個核心 ORM 模型完整設計與實現（10 個資料表，含 configurations / modbus_logs 純 SQL 表） |
| **後端 API** | 100% | 12 個主路由模組 + 14 個子模組完整實現 |
| **前端 UI** | ~98% | 10 個視圖 + 4 個組件（9 個完整，1 個 stub：SystemConfig） |
...
| **統計分析** | 100% | TestResults 報表分析 Tab 完成，ECharts 圖表整合 |
...
| 指標 | 數值 |
|------|------|
| **前端視圖** | 10 個（9 功能完整，1 stub） |
| **前端組件** | 4 個 |
| **前端 API 模組** | 11 個（client + auth + users + projects + tests + testResults + testplans + measurements + analysis + instruments + modbus） |
| **前端 Pinia Stores** | 4 個 |
| **前端 Composables** | 3 個（useMeasurementParams、useTestHistory、useTestTimeline） |
| **前端路由** | 10 條 |
| **API 主路由** | 12 個模組 |
| **API 子路由** | 14 個（testplan 4 + results 7 + modbus 3） |
| **測量模板類型** | 13 種（wait 為 Wait 別名） |
| **儀器驅動** | 27 種（不含 base.py） |
| **ORM 模型** | 9 個 |
| **資料表** | 10 個（含 2 個純 SQL 表：configurations、modbus_logs） |
| **Pydantic Schemas** | 9 個驗證模型 |
| **Alembic 遷移** | 7 個版本 |
| **Core 模組** | 11 個檔案 |
...
> **注意：** `instrument_type` 欄位（UI 選項）與 driver registry key 不同。`ConsoleCommand`/`ComPortCommand`/`TCPIPCommand` 和 `console`/`comport`/`tcpip` 均已在 `INSTRUMENT_DRIVERS` 中建立別名對應，避免 `"No driver for instrument type 'X'"` 錯誤。
...
### TestResults 三合一視圖（v0.9.0）

`/results` → `TestResults.vue` 整合了三個功能區域：

| Tab | 功能 | 來源 |
|-----|------|------|
| **查詢結果** | Session 查詢、分頁、展開明細、Admin 批量刪除、CSV 匯出 | 原 TestResults.vue |
| **歷史趨勢** | ECharts 堆疊長條圖、日期分組、每日 PASS/FAIL/ABORT 統計、Timeline 明細 | 原 TestHistory.vue |
| **報表分析** | Session 統計（mean/median/stdev/MAD）、項目執行時間表、ECharts 趨勢折線圖 | 原 ReportAnalysis.vue |

- 共用 Session 詳情對話框（三個 Tab 共享）
- ECharts 圖表在 Tab 切換時自動重新初始化（處理 DOM 可見性問題）
- Composables（`useTestHistory`、`useTestTimeline`）保持不變，僅視圖層合併

### 時區統一（Asia/Taipei）

所有測試時間戳記統一使用台灣時區，避免 UTC 偏移造成的資料混淆：
...
`test_results` 表的 `wall_time_ms` 欄位記錄前端量測的端到端時間，包含：
...
**前端頁面**：`/results` → `TestResults.vue` 的「報表分析」Tab（ECharts vue-echarts 折線圖）
...
**E2E 測試：**
- `backend/tests/integration/` — 完整端對端測試套件
...
- **modbus/** — 3 個服務檔案（modbus_manager.py、modbus_listener.py、modbus_config.py）

### 完全非同步架構（AsyncSession）

使用 Python asyncio 實現完整的非同步操作：
- **資料庫查詢**：SQLAlchemy async ORM（AsyncSession + asyncmy，完成 Wave 1-4 遷移）
- **Alembic 遷移**：`alembic/env.py` 自動將 `mysql+asyncmy://` 轉換為 `mysql+pymysql://`（同步 pymysql 僅供 Alembic 使用）
- **儀器通訊**：非同步 TCP/Serial
...
若前端 Vite dev server 無法連接後端：
- 確認後端啟動在 port **8765**（非 9100）：`uv run uvicorn app.main:app --reload --host 0.0.0.0 --port 8765`
- `vite.config.js` 的 proxy target 為 `http://localhost:8765`
...
**TestResults 三合一重構**
- **TestHistory.vue → 合併至 TestResults.vue** — 歷史趨勢功能作為 Tab 2 併入 TestResults
- **ReportAnalysis.vue → 合併至 TestResults.vue** — 報表分析功能作為 Tab 3 併入 TestResults
- **路由精簡** — 移除 `/history` 和 `/analysis` 路由（保留 `/results` 統一入口）
- **導航欄更新** — AppNavBar.vue 移除「測試歷史記錄」和「報表分析」按鈕
- **ECharts 圖表生命週期** — Tab 切換時自動 re-init chart（處理 DOM 可見性）
- **共用 Session 詳情對話框** — 三個 Tab 共享結果查看 Dialog
- **Composables 保持不變** — `useTestHistory.js`、`useTestTimeline.js` 僅供 TestResults.vue 使用
...
- **TestHistory.vue 完整** — ECharts 時間軸視圖完整實現（useTestHistory + useTestTimeline composables）
...
- **Async SQLAlchemy 遷移完成**（Wave 1-4）— 全面非同步資料庫架構
  ...
  - Wave 4：測試計劃路由與服務遷移
- TestHistory.vue 測試歷史記錄時間軸視圖（ECharts）
- 儀器管理 Web UI 完整功能
...
**新增功能**
- lowsheen_lib 遷移 Phase 2 & Phase 3 完成（27 種儀器驅動）
- TestResults.vue 測試結果視圖
- AppNavBar.vue 導航欄組件

---

**最後更新**：2026-03-27 | **版本**：v0.9.0 | **狀態**：核心功能完整，Async SQLAlchemy 遷移完成，Modbus 自動觸發測試完成，27 種儀器驅動，13 種測量模板，10 個視圖（9 個完整 + 1 stub），TestResults 三合一視圖，Asia/Taipei 時區統一，生產就緒版本
