    ...
    if cmd in function_mapping:
        ...
        if cmd == 'OPEN' or cmd == 'CLOS':
            ...
            if remote_cmd == None:
                ...
                response_index = instrument.query(str(check_cmd))
                response = '0' if (channels in response_index and cmd == 'OPEN') or (channels not in response_index and cmd == 'CLOS') else '1'
                return response
...
if __name__ == "__main__":
    ...
    # Instrument_value, item, channels, type_ = 'DAQ6510_1', 'OPEN', '101,102', ''

    # args = dict(arg.split('=') for arg in sys.argv[2:])
    # Instrument_value = args.get('Instrument', '')
    # item = args.get('Item', '').upper()
    # channels = args.get('Channel', '')
