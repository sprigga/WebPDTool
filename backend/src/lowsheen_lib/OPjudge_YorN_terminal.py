#!/usr/bin/env python3
"""
OPjudge Yes/No Mode - Terminal Version
Terminal-based binary judgment for headless/Docker environments
...
def prompt_yes_no(content: str) -> str:
    """Prompt operator for Yes/No judgment"""
    print("\n" + "="*60)
    print("OPERATOR JUDGMENT - YES/NO MODE")
    print("="*60)

    if content:
        print(f"\n📋 Question: {content}\n")

    print("Enter your judgment:")
    print("  [Y/yes] → PASS")
    print("  [N/no]  → FAIL")
    print("-"*60)
    ...
    try:
        while True:
            response = input("Your answer (Y/N): ").strip().lower()
...
def main():
    ...
    try:
        ...
        if len(sys.argv) < 3:
            ...
            print("Usage: python3 OPjudge_YorN_terminal.py <test_point_id> <TestParams>",
                  file=sys.stderr)
            print("Example: python3 OPjudge_YorN_terminal.py 'LED_Check' \"['ImagePath=/test.jpg', 'content=Is LED green?']\"",
                  file=sys.stderr)
            sys.exit(1)
