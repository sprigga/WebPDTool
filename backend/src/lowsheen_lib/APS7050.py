...
def send_onlycmd_to_instrument(instrument, cmd):

    # cmd = f'{cmd}\n' 
    # print('send cmd: '+ repr(cmd))
    response = instrument.query(str(cmd))
    # response = instrument.query(f"{cmd}\n")
...
if __name__ == "__main__":
    ...
    # Instrument_value, cmd = 'APS7050_1', '*IDN?\\n'
    ...
    cmd = args['Command'] if 'Command' in args else None
    ...
    if sequence == '--final' :
        ...
        if '\\n' in cmd:
            # print('1 og cmd: '+str(cmd))
            cmd = cmd.replace('\\n', '\n')
        else:
            # print('2 og cmd: '+str(cmd))
            cmd = cmd
        response = send_onlycmd_to_instrument(instrument, cmd)
    else:
        response = send_cmd_to_instrument(instrument, item, channels, type_)
    # response = send_cmd_to_instrument(instrument, cmd, channels, type_)
    # response = send_onlycmd_to_instrument(instrument, cmd)
